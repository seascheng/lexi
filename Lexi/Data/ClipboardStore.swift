...
struct ClipboardItem: Equatable {
    ...
    var previewText: String? {
        ...
        case .text:
            ...
            let collapsed = text.whitespaceCollapsed
...
final class ClipboardStore {
    ...
    static func open(directory: URL, imagesDirectoryName: String) -> ClipboardStore? {
        ...
        // Wipe and retry once. openDatabase may have left a half-open
        // handle on failure — close it before the second attempt.
        store.closeDatabase()
    ...
    /// Close a half-open handle from a failed open attempt — sqlite3_open_v2
    /// allocates a connection object even when it returns an error.
    ...
    func search(_ query: String) -> [ClipboardItem] {
        ...
        if trimmed.count >= 3, db != nil {
            let hits = searchFTS(trimmed)
            let lower = trimmed.lowercased()
            let pinned = items.filter {
                $0.pinnedAt != nil && inMemoryMatch($0, query: lower)
            }
            var seen = Set<UUID>()
            return (pinned + hits).filter { seen.insert($0.id).inserted }
        }
        let lower = trimmed.lowercased()
        return items.filter { inMemoryMatch($0, query: lower) }
    }
    ...
    private func searchFTS(_ query: String) -> [ClipboardItem] {
        ...
        while sqlite3_step(stmt) == SQLITE_ROW {
            if let item = Self.item(from: stmt) { found.append(item) }
        }
        sqlite3_reset(stmt)
        return found
    }
