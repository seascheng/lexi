...
/// Panel/tool icon name → lucide SVG glyph on the 24-unit grid. Template
/// images tint via contentTintColor; an explicit `color` bakes the stroke
/// into the SVG. UNKNOWN ICONS RENDER NIL (blank): a missing icon must be
/// invisible, never a different icon.
func panelIcon(
    for icon: String, title: String, color: NSColor? = nil,
    size: CGFloat = toolbarIconSize
) -> NSImage? {
    let stroke = hexString(color ?? .black)
    let markup = lucideMarkup(for: icon)
        .replacingOccurrences(of: "currentColor", with: stroke)
    let svg = """
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="\(stroke)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">\(markup)</svg>
    """
    guard !markup.isEmpty, let image = NSImage(data: Data(svg.utf8)) else { return nil }
...
func lucideMarkup(for icon: String) -> String {
    ...
    case "sticky-note-fill":
        return """
        <path d="M16 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h11l5-5V5a2 2 0 0 0-2-2Z" fill="currentColor" stroke="currentColor"/><path d="M15 21v-4a2 2 0 0 1 2-2h4"/>
        """
    default:
