...
final class ClipboardMonitor {
    ...
    func start(store: ClipboardStore) {
        ...
        let icons = store.directory.appendingPathComponent("icons", isDirectory: true)
    ...
    /// Bundle ids with no icon (unknown app, render failure) — otherwise
    /// urlForApplication + fileExists re-run every reload for every row.
    private var missingIconBundleIDs: Set<String> = []
    ...
    func cachedIcon(forBundleID bundleID: String?) -> NSImage? {
        ...
        if missingIconBundleIDs.contains(bundleID) { return nil }
        ...
        guard let image = Self.displayImage(fromPNG: NSImage(data: png)) else {
            missingIconBundleIDs.insert(bundleID)
            return nil
        }
        iconCache[bundleID] = image
