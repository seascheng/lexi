...
// ---------------------------------------------------------------------------
// Phase 2 of the full-Swift migration: AI streaming + TTS run inside the
// helper. Rust triggers surfaces (selection, popup) but no longer proxies
// bytes — feature config comes from the shared DB, SSE streams over
// URLSession, and events feed the card renderer in-process.
// ---------------------------------------------------------------------------
...
extension SelectionToolbarApp {
    ...
    func runFeatureLocally(featureId: String, text: String) {
        ...
        let runId = "card-\(Int(Date().timeIntervalSince1970 * 1000))"
        showResultCard(ResultShowPayload(
            runId: runId,
            featureId: featureId,
            title: feature.name,
            icon: feature.icon,
            autoSave: feature.autoSave,
            inputText: text
        ))
        Task { [weak self] in
    ...
    func streamRun(runId: String, feature: LexiAIFeature, text: String) async {
        ...
        do {
            ...
            if isJSON {
                ...
                pushCardEvent(runId, chunk: LexiTranslation.cleanModelText(accumulated), done: true)
            }
        } catch {
    ...
    private func speakSystem(text: String) {
        DispatchQueue.main.async { [weak self] in
            ...
            do {
                ...
                FileLog.write("TTS say failed: \(error.localizedDescription)")
            }
        }
    }
